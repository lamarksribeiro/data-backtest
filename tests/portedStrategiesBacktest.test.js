...
test('cofre-sete seeds lab presets as studio versions', () => {
  const db = openStateDatabase(':memory:');
  bindStrategyLibraryDatabase(db);
  seedPortedStrategies(db);

  const strategy = getStrategyBySlug(db, 'cofre-sete');
  assert.ok(strategy, 'cofre-sete');
  ...
  assert.equal(versions[0].notes, 'Default');
  assert.equal(versions[1].notes, 'BTC · Champion Optimized');
  assert.equal(versions[2].notes, 'V1 Baseline');
  assert.equal(versions[3].notes, 'BTC · Smooth V2');
  ...
  assert.equal(defaultVersion.version, 4);

  const smooth = db.prepare(`
    SELECT validation_json FROM strategy_versions
    WHERE strategy_id = ? AND version = 4
  `).get(strategy.id);
