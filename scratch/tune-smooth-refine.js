...
async function main() {
  ...
  const smooth = { ...base, kellyFraction: 0.11, maxKellyPct: 0.12, maxEntryValue: 6, maxDirectionalPerEvent: 4, cooldownSec: 7 };

  const variants = [
    ['champion', base],
    ['smooth-size', smooth],
    ['smooth-v2', { ...smooth, kellyFraction: 0.12, maxKellyPct: 0.14, maxEntryValue: 6.5 }],
    ['smooth+throttle', { ...smooth, drawdownKellyThrottle: true, drawdownKellyFloor: 0.6, equityPeakThrottlePct: 0.12 }],
    ['smooth+exit', { ...smooth, trailDrop: 0.11, takeProfitPct: 0.38, takeProfitBid: 0.89 }],
    ['smooth-final', { ...smooth, kellyFraction: 0.12, maxKellyPct: 0.13, maxEntryValue: 6, drawdownKellyThrottle: true, drawdownKellyFloor: 0.62, equityPeakThrottlePct: 0.1, trailDrop: 0.11, takeProfitPct: 0.38, takeProfitBid: 0.89, maxDirectionalPerEvent: 4, cooldownSec: 7, riskBudgetPct: 0.4, maxWorstLossAbs: 32 }],
  ];

  for (const [name, params] of variants) {
    console.log(`Rodando ${name}...`);
    const r = await run(loaded, params, db, from, to);
    console.log(`${name.padEnd(16)} | PnL $${r.pnl.toFixed(2).padStart(7)} | DD $${r.dd.toFixed(2).padStart(5)} | PF ${r.pf.toFixed(2)} | Tr ${r.trades} | MaxLoss $${r.maxLoss.toFixed(2)} | Lin ${r.linearity.toFixed(1)}`);
  }
}
