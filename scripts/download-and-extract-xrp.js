...
async function main() {
  console.log('=== BAIXANDO E EXTRAINDO DADOS BINANCE 1S PARA XRPUSDT ===\n');

  for (const dateStr of sampleXrpDates) {
    const zipPath = await downloadBinanceDailyZip('XRPUSDT', dateStr);
    if (!zipPath) continue;

    const csvName = `XRPUSDT-1s-${dateStr}.csv`;
  ...
  console.log('\n[binance-xrp] Processamento de dados 1s da Binance para XRP concluído com sucesso!');
}
