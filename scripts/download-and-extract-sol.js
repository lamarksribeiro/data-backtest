...
async function main() {
  console.log('=== BAIXANDO E EXTRAINDO DADOS BINANCE 1S PARA SOLUSDT ===\n');

  for (const dateStr of sampleSolDates) {
    const zipPath = await downloadBinanceDailyZip('SOLUSDT', dateStr);
    if (!zipPath) continue;

    const csvName = `SOLUSDT-1s-${dateStr}.csv`;
  ...
  console.log('\n[binance-sol] Processamento de dados 1s da Binance para SOL concluído com sucesso!');
}
