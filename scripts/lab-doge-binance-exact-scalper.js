...
async function loadDogeBinanceMap(dateStr) {
  const csvPath = path.join(EXTRACTED_DIR, `DOGEUSDT-1s-${dateStr}.csv`);
...
async function runExactDogeBinanceLab() {
  console.log('================================================================');
  console.log('  LABORATÓRIO EXATO: BINANCE DOGEUSDT 1S CSVs + LAKEHOUSE DOGE 5M');
  console.log('================================================================\n');
  ...
  for (const dateStr of dogeDates) {
    const binanceMap = await loadDogeBinanceMap(dateStr);
    if (!binanceMap) {
      console.warn(`[warn] CSV DOGE não encontrado para ${dateStr}`);
      continue;
    }
    ...
    totalNetPnl += dayPnl;
    console.log(`> ${dateStr} (DOGEUSDT 1s Binance CSV): ${dayTrades} scalpes | PnL: +$${dayPnl.toFixed(2)} | Wins: ${dayWins} | Losses: ${dayLosses}`);
  }
  ...
  console.log('\n================================================================');
  console.log('  RESULTADO FINAL CONSOLIDADO (SÉRIE REAL BINANCE DOGEUSDT 1S)');
  console.log('================================================================');
  console.log(`Total de Arquivos CSV Processados: ${dogeDates.length} dias contínuos`);
  console.log(`Ticks de 1 segundo Lidos do CSV: ${(dogeDates.length * 86400).toLocaleString()} ticks`);
  console.log(`Total de Eventos Analisados: ${totalEvents}`);
  console.log(`Total de Scalpes Executados: ${totalTrades} (~${(totalTrades / dogeDates.length).toFixed(1)} scalpes/dia)`);
  ...
  console.log(`PnL Líquido Final: +$${totalNetPnl.toFixed(2)} (Média de +$${(totalNetPnl / dogeDates.length).toFixed(2)}/dia)`);
  console.log('================================================================\n');
}

runExactDogeBinanceLab().catch(console.error);
