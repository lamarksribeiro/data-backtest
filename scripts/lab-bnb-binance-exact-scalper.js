...
async function loadBnbBinanceMap(dateStr) {
  const csvPath = path.join(EXTRACTED_DIR, `BNBUSDT-1s-${dateStr}.csv`);
...
async function runExactBnbBinanceLab() {
  console.log('================================================================');
  console.log('  LABORATÓRIO EXATO: BINANCE BNBUSDT 1S CSVs + LAKEHOUSE BNB 5M');
  console.log('================================================================\n');
  ...
  for (const dateStr of bnbDates) {
    const binanceMap = await loadBnbBinanceMap(dateStr);
    if (!binanceMap) {
      console.warn(`[warn] CSV BNB não encontrado para ${dateStr}`);
      continue;
    }
    ...
    totalNetPnl += dayPnl;
    console.log(`> ${dateStr} (BNBUSDT 1s Binance CSV): ${dayTrades} scalpes | PnL: +$${dayPnl.toFixed(2)} | Wins: ${dayWins} | Losses: ${dayLosses}`);
  }
  ...
  console.log('\n================================================================');
  console.log('  RESULTADO FINAL CONSOLIDADO (SÉRIE REAL BINANCE BNBUSDT 1S)');
  console.log('================================================================');
  console.log(`Total de Arquivos CSV Processados: ${bnbDates.length} dias contínuos`);
  console.log(`Ticks de 1 segundo Lidos do CSV: ${(bnbDates.length * 86400).toLocaleString()} ticks`);
  console.log(`Total de Eventos Analisados: ${totalEvents}`);
  console.log(`Total de Scalpes Executados: ${totalTrades} (~${(totalTrades / bnbDates.length).toFixed(1)} scalpes/dia)`);
  ...
  console.log(`PnL Líquido Final: +$${totalNetPnl.toFixed(2)} (Média de +$${(totalNetPnl / bnbDates.length).toFixed(2)}/dia)`);
  console.log('================================================================\n');
}

runExactBnbBinanceLab().catch(console.error);
