...
async function main() {
  console.log('=== BAIXANDO E EXTRAINDO DADOS BINANCE 1S PARA BNBUSDT ===\n');

  for (const dateStr of sampleBnbDates) {
    const zipPath = await downloadBinanceDailyZip('BNBUSDT', dateStr);
    if (!zipPath) continue;

    const csvName = `BNBUSDT-1s-${dateStr}.csv`;
  ...
  console.log('\n[binance-bnb] Processamento de dados 1s da Binance para BNB concluído com sucesso!');
}
