...
async function main() {
  console.log('=== BAIXANDO E EXTRAINDO DADOS BINANCE 1S PARA DOGEUSDT ===\n');

  for (const dateStr of sampleDogeDates) {
    const zipPath = await downloadBinanceDailyZip('DOGEUSDT', dateStr);
    if (!zipPath) continue;

    const csvName = `DOGEUSDT-1s-${dateStr}.csv`;
  ...
  console.log('\n[binance-doge] Processamento de dados 1s da Binance para DOGE concluído com sucesso!');
}
