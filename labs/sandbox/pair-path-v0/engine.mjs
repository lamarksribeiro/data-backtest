/**
 * Pair-Path V0 / Clip-Path V1 — pure event engine (no I/O).
 * Market-adapted: open + hedge (+ optional EQ), taker_limit+miss, no MULT/rearm.
 * Clip-Path: optional hedgeLevels for multi-clip hedge (2–3 níveis DESC).
 */
...
export const DEFAULT_PARAMS = {
  ...
  makerTimeoutSec: 30,
  /**
   * `cross` preserves the legacy full-maker-fill proxy. `none` is the
   * conservative choice when journals have top-of-book but no queue/depth.
   */
  restingFillModel: 'cross',
  /** Require N consecutive qualifying journal ticks before a taker fill. */
  confirmationTicks: 1,
  /** Legacy filled every eligible clip on one tick; live submits sequentially. */
  maxClipsPerTick: Number.POSITIVE_INFINITY,
  eqAskMax: 0.05,
  eqAvgSumMax: 0.99,
  tauEqMin: 8,
  maxEventNotional: 25,
