...
const DEFAULT_PARAMS = {
  ...
  entropyCompressionCap: 0.94,
  quantumObiFactor: 0.85,
  quantumCoherenceHighThreshold: 0.78,
  quantumCoherenceLowThreshold: 0.52,
  quantumSizingHighFactor: 1.50,
  quantumSizingLowFactor: 0.60,
  temperatureStopScale: 0.12,
};
...
function mergeGammaLadderParams(raw = {}) {
  ...
  const numericKeys = [
    ...
    'minTicksBeforeEntry',
    'entropyCompressionCap', 'quantumObiFactor', 'quantumCoherenceHighThreshold',
    'quantumCoherenceLowThreshold', 'quantumSizingHighFactor', 'quantumSizingLowFactor',
    'temperatureStopScale',
  ];
  ...
  params.entropyCompressionCap = clamp(params.entropyCompressionCap, 0.01, 1.50);
  params.quantumObiFactor = clamp(params.quantumObiFactor, 0.01, 2.0);
  params.quantumCoherenceHighThreshold = clamp(params.quantumCoherenceHighThreshold, 0.01, 2.0);
  params.quantumCoherenceLowThreshold = clamp(params.quantumCoherenceLowThreshold, 0.01, 2.0);
  params.quantumSizingHighFactor = clamp(params.quantumSizingHighFactor, 0.10, 5.0);
  params.quantumSizingLowFactor = clamp(params.quantumSizingLowFactor, 0.01, 2.0);
  params.temperatureStopScale = clamp(params.temperatureStopScale, 0.0, 2.0);
...
function createBacktestRunner(rawParams = {}) {
  ...
  const directionalSize = (candidate, tick) => {
    const equityNow = Math.max(0, params.walletSize + totalPnl);
    ...
    // Quantum Coherence Sizing
    const OBI = orderBookImbalance(candidate.side, tick, 5);
    const quantumCoherence = candidate.probability * (1.0 + (OBI * params.quantumObiFactor));
    let quantumSizingFactor = 1.0;
    if (quantumCoherence >= params.quantumCoherenceHighThreshold) {
      quantumSizingFactor = params.quantumSizingHighFactor;
    } else if (quantumCoherence <= params.quantumCoherenceLowThreshold) {
      quantumSizingFactor = params.quantumSizingLowFactor;
    }
    
    const kellyValue = equityNow * Math.min(params.maxKellyPct, kelly * params.kellyFraction) * quantumSizingFactor;
  ...
  const attemptDirectionalEntry = (tick) => {
    ...
    // Shannon Entropy Filter (Bloqueio de Ruído Caótico)
    const samples = current.samples;
    if (samples.length >= params.minTicksBeforeEntry) {
      const latest = samples[samples.length - 1];
      const p0 = toFiniteNumber(tick.btc_price);
      if (p0 != null) {
        const p5Sample = sampleAgo(samples, 5) || samples[0] || latest;
        const p15Sample = sampleAgo(samples, 15) || p5Sample;
        const p30Sample = sampleAgo(samples, 30) || p15Sample;
        const p5 = p5Sample.btc ?? p0;
        const p15 = p15Sample.btc ?? p5;
        const p30 = p30Sample.btc ?? p15;

        const up0 = (p0 > p5) ? 1.0 : 0.0;
        const up1 = (p5 > p15) ? 1.0 : 0.0;
        const up2 = (p15 > p30) ? 1.0 : 0.0;
        const p_up = (up0 + up1 + up2) / 3.0;
        const shannonEntropy = 4.0 * p_up * (1.0 - p_up);
        
        if (shannonEntropy > params.entropyCompressionCap) {
          return false; // Bloqueado por ruído entrópico
        }
      }
    }
    ...
    const targetValue = directionalSize(candidate, tick);
  ...
  const maybeProcessPositions = (tick) => {
    ...
    for (const side of ['UP', 'DOWN']) {
      ...
      // Thermo-Adaptive Stop & Trailing
      const vol = recentVol(current.samples, params.volLookbackSec);
      const volAdapt = vol * params.temperatureStopScale;
      const activeTrailDrop = clamp(params.trailDrop * (1.0 + volAdapt), 0.01, 0.40);
      const activeStopLossBid = clamp(params.stopBid * (1.0 - volAdapt), 0.01, 0.30);

      if (sideInventory.maxBid >= params.trailAfterBid && sideInventory.maxBid - bid >= activeTrailDrop) {
        sellSide(tick, side, sideInventory.remainingQty, bid, 'trailing gamma', bid >= avgOpen ? 'profit' : 'stop');
        continue;
      }

      if (bid <= activeStopLossBid && edgeToBid < params.edgeExitBelow && timeRemainingSec > params.lateExitSec) {
