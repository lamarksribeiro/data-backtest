 ...
 * LRU cache de batches de ticks convertidos (re-run com mesma janela).

 * Armazena batches em formato colunar (Float64Array para campos numéricos).
...
class DatasetCache {
  ...
  get(key) {
    ...
    return materializeBatches(entry.batches);
  ...
  set(key, batches) {
    ...
    const columnarBatches = batches.map(toColumnarBatch);

    const size = estimateSize(columnarBatches);
    ...
    this.map.set(key, { batches: columnarBatches, size });
...
function toColumnarBatch(batch) {
...
function materializeBatches(batches) {
...
function estimateSize(batches) {
  ...
  for (const batch of batches) {
    ...
    for (const key of batch.keys) {
      ...
      bytes += column instanceof Float64Array ? column.byteLength : column.length * 32;
...
export { toColumnarBatch, fromColumnarBatch, materializeBatches };
