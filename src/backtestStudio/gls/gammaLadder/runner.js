import { applyStopReverseParams, stopReverseBudget, stopReverseTrigger } from '../../../strategies/stopReverse.js';
...
export function createGammaLadderBacktestRunner(rawParams = {}) {
...
export function runGammaLadderBacktest(rawParams, ticks) {
  const runner = createGammaLadderBacktestRunner(rawParams);
  for (const tick of ticks) runner.processTick(tick);
  return runner.finish();
}

export async function runGammaLadderBacktestInBatches(rawParams, tickBatches) {
  const runner = createGammaLadderBacktestRunner(rawParams);
